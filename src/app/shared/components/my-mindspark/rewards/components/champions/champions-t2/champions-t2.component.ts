    ...
    if (championsInfoChanges !== undefined && championsInfoChanges !== null) {
      ...
      this.championsInfo.forEach((champ) => {
        ...
        const sparkie = {
          ...
          othercount: newChamp.otherSparkie
        };
        const shield = {
          starOftheMonth: {
            name: newChamp.maxShield[0].name,
            count: newChamp.maxShield[0].count,
            thisUser: newChamp.maxShield[0].thisUser,
          },
          othernames: this.getMoreChamps(newChamp.maxShield),
          othercount: newChamp.otherShield
        };
        const achievement = {
          starOftheMonth: {
            name: newChamp.maxAchievements[0].name,
            count: newChamp.maxAchievements[0].count,
            thisUser: newChamp.maxAchievements[0].thisUser,
          },
          othernames: this.getMoreChamps(newChamp.maxAchievements),
          othercount: newChamp.otherAchievement
        };
        champion = {
